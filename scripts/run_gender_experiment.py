"""Run complete SparC³ gender bias removal experiment

This script orchestrates the full pipeline:
1. Load attribution scores (general + gender bias)
2. Compute differential and identify neurons to prune
3. Load model and prune neurons
4. Evaluate before/after (perplexity + gender bias)
5. Save results
...
Usage:
    python scripts/run_gender_experiment.py \\
        ...
        --output results/gender_experiment/
"""
import sys
sys.path.append('.')
...
from src.evaluation import evaluate_perplexity, evaluate_gender_bias
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Run complete SparC³ gender bias removal experiment',
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to gender bias attribution scores'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to gender bias prompts pickle file'
    )
...
def main():
    ...
    print("="*70)
    print("SparC³ Gender Bias Removal Experiment")
    print("="*70)
    ...
    # Load general scores (average if multiple - REUSE from toxicity experiment!)
    ...
    # Load gender bias scores
    print(f"\nLoading gender bias scores from {args.gender_scores}...")
    gender_scores = torch.load(args.gender_scores)
    print(f"✓ Loaded {len(gender_scores)} layers")
    ...
    diff_scores = compute_differential_scores(general_scores, gender_scores)
    ...
    # Save neuron indices
    neuron_file = output_dir / f"pruned_neurons_gender_{timestamp}.json"
    save_neuron_indices(
        neurons_to_prune,
        str(neuron_file),
        metadata={
    ...
    if not args.skip_baseline:
        ...
        gender_prompts = load_samples(args.gender_prompts)
        wikitext = load_wikitext2(split='test')
        print(f"✓ Loaded {len(gender_prompts)} gender bias prompts")
        ...
        print("\n2. Evaluating gender bias...")
        baseline_bias, baseline_scores, baseline_completions = evaluate_gender_bias(
            model, tokenizer, gender_prompts, verbose=True
        )

        baseline_results = {
            'perplexity': float(baseline_ppl),
            'gender_bias_avg': float(baseline_bias),
            'gender_bias_std': float(np.std(baseline_scores)),
            'gender_bias_scores': [float(s) for s in baseline_scores],
        ...
        baseline_file = output_dir / f"baseline_eval_gender_{timestamp}.json"
    ...
    if args.save_model:
        model_path = output_dir / f"pruned_model_gender_{timestamp}"
    ...
    # Evaluate gender bias
    print("\n2. Evaluating gender bias...")
    pruned_bias, pruned_scores, pruned_completions = evaluate_gender_bias(
        model, tokenizer, gender_prompts, verbose=True
    )

    pruned_results = {
        'perplexity': float(pruned_ppl),
        'gender_bias_avg': float(pruned_bias),
        'gender_bias_std': float(np.std(pruned_scores)),
        'gender_bias_scores': [float(s) for s in pruned_scores],
    ...
    # Save pruned results
    pruned_file = output_dir / f"pruned_eval_gender_{timestamp}.json"
    ...
    # ========================================================================
    print("\n" + "="*70)
    print("FINAL RESULTS - GENDER BIAS REMOVAL")
    print("="*70)

    if not args.skip_baseline:
        baseline_ppl = results['baseline']['perplexity']
        baseline_bias = results['baseline']['gender_bias_avg']
        pruned_ppl = results['pruned']['perplexity']
        pruned_bias = results['pruned']['gender_bias_avg']
        ...
        results['improvements'] = {
            ...
            'gender_bias_change_pct': float(bias_change)
        }
    else:
        print(f"\nPruned Model Results:")
        print(f"  Perplexity: {results['pruned']['perplexity']:.2f}")
        print(f"  Gender Bias: {results['pruned']['gender_bias_avg']:.4f}")

    # Save complete results
    results_file = output_dir / f"experiment_results_gender_{timestamp}.json"
    ...
    print("\n" + "="*70)
    print("✅ GENDER BIAS EXPERIMENT COMPLETE")
    print("="*70)


if __name__ == "__main__":
    import numpy as np  # Import here for use in main()
    main()
